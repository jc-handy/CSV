...
* END: Line ending. This can be C for carraige return (\\r), N for newline
       (\\n), or B for both (\\r\\n). L for linefeed is the same as N for
       newline (\\n). Any other character is taken litterally and will be
       interpreted as end-of-line in an input file and will terminate each row
...
quoting_map = dict(A=QUOTE_ALL, M=QUOTE_MINIMAL, N=QUOTE_NONNUMERIC, X=QUOTE_NONE)
...
def parse_dialect(dialect_name, dialect_spec):
    ...
    if len(dialect_spec) < len(DEFAULT_DIALECT_SPEC):
        dialect_spec += DEFAULT_DIALECT_SPEC[
            -(len(DEFAULT_DIALECT_SPEC) - len(dialect_spec)) :
        ]
    ...
    try:
        ...
        raise Error(
            f'Bad "quoting" value in dialect specification string: {dialect_spec!r}'
        )
    try:
        d["lineterminator"] = lineterminator_map[d["lineterminator"]]
    except KeyError:
        raise Error(
            f'Bad "lineterminator" value in dialect specification string: {dialect_spec!r}'
        )
    if d["delimiter"] == "n":
        d["delimiter"] = None
    if d["escapechar"] == "N":
    ...
    d["lineterminator"] = lineterminator_map.get(
        d["lineterminator"], d["lineterminator"]
    )
